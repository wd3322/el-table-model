<template>
  <div v-if="form">
    <el-form-item
      :prop="`list.${scope.$index}.${column.prop}`"
      :rules="form.rules"
      :row-index="scope.$index"
    >
      <span @click.stop>
        <component
          :is="{
            input: 'el-input',
            autocomplete: 'el-autocomplete',
            count: 'el-input-number',
            select: 'el-select',
            cascader: 'el-cascader',
            time: 'el-time-picker',
            date: 'el-date-picker',
            dates: 'el-date-picker',
            datetime: 'el-date-picker',
            month: 'el-date-picker',
            year: 'el-date-picker'
          }[form.type]"
          ref="editable"
          class="el-editable-form"
          :style="{ width: form.width }"
          v-model.trim="scope.row[column.prop]"
          v-bind="getAttrs('form-item', column, scope)"
          v-on="form.events"
        >
          <template v-if="form.type === 'select' && form.options">
            <el-option
              v-for="(option, optionsIndex) in form.options"
              :key="optionsIndex"
              v-bind="option"
            >
              <span>{{ option.label }}</span>
            </el-option>
          </template>
        </component>
      </span>
    </el-form-item>
  </div>
</template>

<script>
import Utils from '../utils.js'

export default {
  name: 'ElTableModelColumn',
  props: {
    column: {
      type: Object,
      required: true,
      default: () => ({})
    },
    scope: {
      type: Object,
      required: true,
      default: () => ({})
    }
  },
  data() {
    return {
      form: null
    }
  },
  created() {
    const { row, column, $index } = this.scope
    const form = Utils.getPrototype(this.column.form).indexOf('function') !== -1
      ? this.column.form(row, column, row[column.property], $index)
      : this.column.form
    if (Utils.getPrototype(form) === 'object') {
      this.form = form
    }
  },
  methods: {
    getAttrs(type, column, scope) {
      let result = {}
      if (type === 'form-item') {
        const appendProps = []
        const excludeProps = []
        result = {
          size: 'mini',
          placeholder: column.label || '',
          ...this.form
        }
        if (['count', 'select', 'cascader', 'time'].includes(this.form.type)) {
          appendProps.push('type')
        }
        if (['select'].includes(this.form.type)) {
          appendProps.push('options')
        }
        if (['cascader'].includes(this.form.type)) {
          result.props = this.form.props
          excludeProps.push('props')
        }
        const props = ['width', 'rules', 'events', ...appendProps]
        for (const prop of props) {
          if (result.hasOwnProperty(prop) && !excludeProps.includes(prop)) {
            delete result[prop]
          }
        }
      }
      return result
    }
  }
}
</script>
